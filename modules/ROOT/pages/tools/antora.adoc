...
Conditional directives in AsciiDoc help filter content based on output. This feature is most helpful for brief amounts of content that don't warrent their own page. Conditional directives let you single-source your docs while differentiating online vs. print content or only showing certain content to an audience.

ifdef: If you don't delcare the attribute, it's not included in the output. You can declare it in your `antora-playbook.yml`, page header, or in your command line call (my preference for flexibility), it's not included.
...
To build output that includes these audiences, you can set the desired attribute in your `antora-playbook.yml` or in your terminal command, like `npx antora --fetch --attribute=admin antora-playbook.yml`

NOTE: You can specify multiple attributes with comma separation: `npx antora --fetch --attribute=admin,internal playbook.yml`.
...
Here's the call for how I made my portfolio: `asciidoctor-pdf --theme themes/pdf-theme.yml -a pdf-fontsdir=themes/fonts/OpenSans resume.adoc -o modules/ROOT/attachments/resume.pdf`
