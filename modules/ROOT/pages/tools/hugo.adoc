...
Hugo is a fast and powerful static site generator for writing in plain text. This syntax is simpler to read and write with than Flare's messy XHTML, but don't let this simplicity fool you. Hugo's has an incredibly high ceiling for configuration, since it leverages and extends the Go programming language. This software's main strengths are its templating and ability to parse content for xref:#_shortcodes[shortcodes] and xref:#_.Getpage_method[methods].

And since Hugo is open source, I can experiment with it for free in personal projects, if I choose. This is a huge advantage compared to a pricey proprietary authoring tool like Flare.
...
Just as the name suggests, shortcodes use quick and clean syntax to refer to more complex functions. This maintains docs readability without sacrificing complexity. A one-line `{{< shortcode >}}` can run dozens of lines of code behind the scenes. By abstracting away this complexity in a dedicated file, Hugo lets you centralize the management of shortcodes, while keeping them simple to reference.
...
Flexibility is an asset, and the ability to fine-tune templates to handle a variety of scenarios is an advantage. Who wants to manage several different templates when one does the trick?

== Front matter

Hugo pages use front matter to set values you can parse and refer to as metadata in a variety of ways. Standard values for front matter are `Title`, `Description`, `Date`, and `Author` for example. Though you can further define metadata in front matter however best suits you. 
...
Reuse content you've already written by grabbing it with the `.GetPage` method in Hugo. A classic example of this is when you want to introduce a feature on release notes when you've already written a description elsewhere. By storing this description in the feature's front matter, you can grab it with the `.Getpage` method.
...
Then, add whatever tags you'd like to a content page, for example "best practice." Hugo automatically generates a page for each tag you define at `/tags/<tag-name>/`. 
...
Finally, update your page template to display a page's tags. Users that click a tag are redirected to the taxonomy page selected.
