...
When writing with AsciiDoc in *Antora*, I can use the `include::` directive to display source code from a file, rather than copying and pasting it on the page. If I want to reuse this example elsewhere and the example code is updated, all reference to it update automatically.
...
The tightest feedback loop should be between a SME and technical writer. Ask the SME how they'd prefer to handle reviews and accommodate accordingly. They're helping you out, after all! This review ensures technical accuracy, proper order of operations, and prerequisites. This review can be completed prior to or as part of a GitHub Pull Request.
...
Validating documentation doesn't need to be purely subjective. Language might not follow as strict of conventions as code, but I like to test my docs with Vale for style guide adherence and readability metrics. Style guide linting in Vale can be adjusted or extended to reflect your organization's preferences, as well. 
...
NOTE: Running the `vale sync` command will re-download any removed rules from your list of packages.

=== Acceptance criteria

Another test of documentation is to verify the writing against the ticket's acceptance criteria. This calls for clear, demonstrative acceptance criteria that can pass or fail such a test. Here's a couple of examples to show what I mean: 
