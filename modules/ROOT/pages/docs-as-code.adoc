...
I prefer to name branches using ticket IDs, for example [branch]`feature/jira-123`, as descriptive names can become confusing, vague, or even inaccurate as development evolves. What is lost in presentation is gained in clarity, in my experience. Used this way, the associated ticket ID serves as a breadcrumb to the source of truth if someone is curious about scope or feature details.
