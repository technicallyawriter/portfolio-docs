...
As a general rule, [branch]`master` branch should always contain production-ready code. This means that documentation for upcoming releases get worked in feature branches that are typically scoped at the link:agile.adoc[Epic or Feature level]. 

.Feature branches (simple) 
image::feature-branch.png[Feature branches without release branch]

Branching in this way is more straight-forward the more frequent a company's release schedule is. However, if a company prefers a longer release runway (_cough_ waterfall! _cough_), I'll create a release branch off [branch]`main` to merge completed feature branches into. Branching like this makes writing release notes for service packs and hotfixes simple, as these docs are just another feature branch. 

.Feature branches with a release branch
image::feature-release-branches.png[Feature branches with release branch]
